...
- (void)setModel:(LLPhotoBrowserModel *)model
{
    ...
    if (model) {
        ...
        self.imageRequestID = [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestImageForAsset:model.asset
                                                                                                  size:size
                                                                                       progressHandler:nil
        ...
        if (model.type == LLAssetMediaTypeVideo || model.type == LLAssetMediaTypeNetVideo) {
...
- (UIButton *)button
{
    if (!_button) {
        _button = ({
            ...
            [object setImage:[UIImage ll_loadBundleImage:@"zl_btn_unselected"
                                            ClassName:NSStringFromClass([self class]) bundleName:@"LSJHPhotoBrowser"]
                    forState:UIControlStateNormal];
            [object setImage:[UIImage ll_loadBundleImage:@"zl_btn_selected"
                                            ClassName:NSStringFromClass([self class]) bundleName:@"LSJHPhotoBrowser"]
...
- (UIImageView *)videoImageView
{
    if (!_videoImageView) {
        _videoImageView = ({
            UIImageView * object = [[UIImageView alloc] init];
            object.image =
            [UIImage ll_loadBundleImage:@"video"
                           ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"];
            object;
        });
    }
