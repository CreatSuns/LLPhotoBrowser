...
- (LLPreviewView *)previewView
{
    if (!_previewView) {
        _previewView                  = [[LLPreviewView alloc] initWithFrame:self.bounds];
...
- (void)setModel:(LLPhotoBrowserModel *)model
{
    _model                         = model;
...
@implementation LLPreviewView

- (void)layoutSubviews
{
    [super layoutSubviews];
    if (self.model.type == LLAssetMediaTypeImage ||
        self.model.type == LLAssetMediaTypeGif ||
        (self.model.type == LLAssetMediaTypeLivePhoto && !self.showLivePhoto) ||
        self.model.type == LLAssetMediaTypeNetImage) {
        self.imageGifView.frame = self.bounds;
    } else if (self.model.type == LLAssetMediaTypeLivePhoto) {
        self.livePhotoView.frame = self.bounds;
    } else if (self.model.type == LLAssetMediaTypeVideo) {
        self.videoView.frame = self.bounds;
    } else if (self.model.type == LLAssetMediaTypeNetVideo) {
        self.netVideoView.frame = self.bounds;
    }
}

- (LLPreviewImageAndGif *)imageGifView
{
    if (!_imageGifView) {
        _imageGifView                   = [[LLPreviewImageAndGif alloc] initWithFrame:self.bounds];
...
- (LLPreviewLivePhoto *)livePhotoView
{
    if (!_livePhotoView) {
        _livePhotoView                   = [[LLPreviewLivePhoto alloc] initWithFrame:self.bounds];
        _livePhotoView.singleTapCallBack = self.singleTapCallBack;
    }
    return _livePhotoView;
}

- (LLPreviewVideo *)videoView
{
    if (!_videoView) {
        _videoView                   = [[LLPreviewVideo alloc] initWithFrame:self.bounds];
        _videoView.singleTapCallBack = self.singleTapCallBack;
    }
    return _videoView;
}

- (LLPreviewNetVideo *)netVideoView
{
    if (!_netVideoView) {
        _netVideoView                   = [[LLPreviewNetVideo alloc] initWithFrame:self.bounds];
...
- (void)setModel:(LLPhotoBrowserModel *)model
{
    ...
    switch (model.type) {
        ...
        case LLAssetMediaTypeGif: {
            [self addSubview:self.imageGifView];
            [self.imageGifView loadNormalImage:model.asset];
        } break;
        case LLAssetMediaTypeLivePhoto: {
...
- (void)reload
{
    if (self.showGif &&
        ...
        [self.imageGifView loadGifImage:self.model.asset];
               ...
               self.model.type == LLAssetMediaTypeLivePhoto) {
...
- (void)resumePlay
{
    if (self.model.type == LLAssetMediaTypeGif) {
...
@implementation LLBasePreviewView
...
- (void)layoutSubviews
{
    ...
    CGFloat width        = 40;
    CGFloat x            = (self.cmam_width - width) / 2;
    CGFloat y            = (self.cmam_height - width) / 2;
    self.indicator.frame = CGRectMake(x, y, width, width);
}

- (LLProgressView *)indicator
{
    if (!_indicator) {
        _indicator        = [[LLProgressView alloc] init];
...
@interface LLPreviewImageAndGif () <UIScrollViewDelegate>

@property (nonatomic, assign) BOOL isGif;

@end

@implementation LLPreviewImageAndGif
...
- (void)loadGifImage:(PHAsset *)asset
{
    ...
    [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestOriginalImageDataForAsset:asset
    ...
    completion:^(NSData * data, NSDictionary * info) {
        ...
        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
            strongSelf.indicator.hidden = YES;
            strongSelf.imageView.image  = [[LLPhotoBrowserManager sharedPhotoBrowserManager] transformToGifImageWithData:data];
...
- (void)loadNormalImage:(PHAsset *)asset
{
    ...
    WeakSelf(weakSelf);
    self.imageRequestID = [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestImageForAsset:asset
    size:[self requestImageSize:asset]
    progressHandler:^(double progress, NSError * error, BOOL * stop, NSDictionary * info) {
        StrongSelf(strongSelf);
        dispatch_async(dispatch_get_main_queue(), ^{
            strongSelf.indicator.progress = progress;
...
- (void)loadImage:(id)obj
{
    ...
    if ([obj isKindOfClass:UIImage.class]) {
        ...
        WeakSelf(weakSelf);
        [self.imageView sd_setImageWithURL:obj
...
- (void)resetSubviewSize:(id)obj
{
    ...
    CGFloat width = MIN(ScreenWidth, w);

    if (isLandscape) {
        CGFloat height    = MIN(self.cmam_height, h);
        frame.origin      = CGPointZero;
        frame.size.height = height;
        UIImage * image   = self.imageView.image;

        CGFloat imageScale  = image.size.width / image.size.height;
        CGFloat screenScale = ScreenWidth / self.cmam_height;

        if (imageScale > screenScale) {
            frame.size.width = floorf(height * imageScale);
            if (frame.size.width > ScreenWidth) {
                frame.size.width  = ScreenWidth;
                frame.size.height = ScreenWidth / imageScale;
            }
        } else {
            CGFloat width = floorf(height * imageScale);
            if (width < 1 || isnan(width)) {
                //iCloud图片height为NaN
                width = self.cmam_width;
            }
        ...
        CGFloat screenScale = self.cmam_height / ScreenWidth;
    ...
    CGSize contentSize;
    if (!isLandscape) {
        contentSize = CGSizeMake(width, MAX(self.cmam_height, frame.size.height));
        if (frame.size.height < self.cmam_height) {
            self.containerView.center = CGPointMake(self.cmam_width / 2, self.cmam_height / 2);
        } else {
            self.containerView.frame = (CGRect){CGPointMake((self.cmam_width - frame.size.width) / 2, 0), frame.size};
        }
    } else {
        contentSize = frame.size;
        if (frame.size.width < self.cmam_width ||
            frame.size.height < self.cmam_height) {
            self.containerView.center = CGPointMake(self.cmam_width / 2, self.cmam_height / 2);
        }
    }
...
{
    CGFloat offsetX           = (scrollView.cmam_width > scrollView.contentSize.width) ? (scrollView.cmam_width - scrollView.contentSize.width) * 0.5 : 0.0;
    CGFloat offsetY           = (scrollView.cmam_height > scrollView.contentSize.height) ? (scrollView.cmam_height - scrollView.contentSize.height) * 0.5 : 0.0;
...
@implementation LLPreviewLivePhoto
...
{
    WeakSelf(weakSelf);
    [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestLivePhotoForAsset:asset
                                                                      completion:^(PHLivePhoto * lv, NSDictionary * info) {
                                                                          StrongSelf(strongSelf);
                                                                          if (lv) {
...
- (UIButton *)playBtn
{
    if (!_playBtn) {
        ...
        [_playBtn setImage:[UIImage ll_loadBundleImage:@"icon_begin" ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"] forState:UIControlStateNormal];
        _playBtn.frame = CGRectMake(0, 64, self.cmam_width, self.cmam_height - 64 - 44);
...
- (void)loadNormalImage:(PHAsset *)asset
{
    ...
    if (![[LLPhotoBrowserManager sharedPhotoBrowserManager] judgeAssetisInLocalAblum:asset]) {
...
{
    WeakSelf(weakSelf);
    [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestVideoForAsset:asset
...
- (void)stopPlayVideo
{
    ...
    if (player.rate != .0) {
        [player pause];
        [self.playBtn setImage:[UIImage ll_loadBundleImage:@"icon_begin" ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"] forState:UIControlStateNormal];
    }
}
...
@implementation LLPreviewNetVideo {
