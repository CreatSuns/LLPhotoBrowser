...
@implementation LLPhotoBrowserManager

+ (instancetype)sharedPhotoBrowserManager
{
    static LLPhotoBrowserManager * manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[LLPhotoBrowserManager alloc] init];
...
#pragma mark - 获取资源
- (NSArray<LLPhotoBrowserModel *> *)getAllAssetInPhotoAlbumWithAscending:(BOOL)ascending limitCount:(NSInteger)limit allowSelectVideo:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage allowSelectGif:(BOOL)allowSelectGif allowSelectLivePhoto:(BOOL)allowSelectLivePhoto
{
...
- (LLPhotoBrowserAlbumModel *)getCameraRollAlbumList:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage
{
    PHFetchOptions * option                         = [[PHFetchOptions alloc] init];
    ...
    __block LLPhotoBrowserAlbumModel * m;
...
- (void)getCameraRollAlbumList:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage complete:(void (^)(LLPhotoBrowserAlbumModel *))complete
{
    if (complete) {
...
- (void)getPhotoAblumList:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage complete:(void (^)(NSArray<LLPhotoBrowserAlbumModel *> *))complete
{
     ...
     */
    NSMutableArray<LLPhotoBrowserAlbumModel *> * arrAlbum = [NSMutableArray array];
    for (PHFetchResult<PHAssetCollection *> * album in arrAllAlbums) {
        [album enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL * stop) {
            ...
            if (collection.assetCollectionSubtype == PHAssetCollectionSubtypeSmartAlbumUserLibrary) {
                //所有照片
                LLPhotoBrowserAlbumModel * m = [self getAlbumModeWithTitle:title result:result allowSelectVideo:allowSelectVideo allowSelectImage:allowSelectImage];
                m.isCameraRoll                = YES;
...
//获取相册列表model
- (LLPhotoBrowserAlbumModel *)getAlbumModeWithTitle:(NSString *)title result:(PHFetchResult<PHAsset *> *)result allowSelectVideo:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage
{
    LLPhotoBrowserAlbumModel * model = [[LLPhotoBrowserAlbumModel alloc] init];
    model.title                       = title;
...
- (NSArray<LLPhotoBrowserModel *> *)getPhotoInResult:(PHFetchResult<PHAsset *> *)result allowSelectVideo:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage allowSelectGif:(BOOL)allowSelectGif allowSelectLivePhoto:(BOOL)allowSelectLivePhoto
{
...
- (NSArray<LLPhotoBrowserModel *> *)getPhotoInResult:(PHFetchResult<PHAsset *> *)result allowSelectVideo:(BOOL)allowSelectVideo allowSelectImage:(BOOL)allowSelectImage allowSelectGif:(BOOL)allowSelectGif allowSelectLivePhoto:(BOOL)allowSelectLivePhoto limitCount:(NSInteger)limit
{
    NSMutableArray<LLPhotoBrowserModel *> * arrModel = [NSMutableArray array];
    __block NSInteger count                           = 1;
    [result enumerateObjectsUsingBlock:^(PHAsset * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        LLAssetMediaType type = [self transformAssetType:obj];

        if (type == LLAssetMediaTypeImage && !allowSelectImage) return;
        if (type == LLAssetMediaTypeGif && !allowSelectImage) return;
        if (type == LLAssetMediaTypeLivePhoto && !allowSelectImage) return;
        if (type == LLAssetMediaTypeVideo && !allowSelectVideo) return;
        ...
        [arrModel addObject:[LLPhotoBrowserModel modelWithAsset:obj type:type duration:duration]];
        count++;
    }];
    return arrModel;
}
...
- (LLAssetMediaType)transformAssetType:(PHAsset *)asset
{
    switch (asset.mediaType) {
        case PHAssetMediaTypeAudio:
            return LLAssetMediaTypeAudio;
        case PHAssetMediaTypeVideo:
            return LLAssetMediaTypeVideo;
        case PHAssetMediaTypeImage:
            if ([[asset valueForKey:@"filename"] hasSuffix:@"GIF"]) return LLAssetMediaTypeGif;

            if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive || asset.mediaSubtypes == 10) return LLAssetMediaTypeLivePhoto;

            return LLAssetMediaTypeImage;
        default:
            return LLAssetMediaTypeUnknown;
    }
}
...
- (void)requestSelectedImageForAsset:(LLPhotoBrowserModel *)model isOriginal:(BOOL)isOriginal allowSelectGif:(BOOL)allowSelectGif completion:(void (^)(UIImage *, NSDictionary *))completion
{
    if (model.type == LLAssetMediaTypeGif && allowSelectGif) {
        [self requestOriginalImageDataForAsset:model.asset
                               progressHandler:nil
                                    completion:^(NSData * data, NSDictionary * info) {
                                        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
                                            UIImage * image = [[LLPhotoBrowserManager sharedPhotoBrowserManager] transformToGifImageWithData:data];
                                            if (completion) {
...
{
    PHCachingImageManager * manager = [LLPhotoBrowserManager sharedPhotoBrowserManager].cachingManager;
...
- (void)exportEditVideoForAsset:(AVAsset *)asset range:(CMTimeRange)range type:(LLExportVideoType)type complete:(void (^)(BOOL, PHAsset *))complete
{
    [self export:asset
                range:range
                 type:type
       ...
       watermarkImage:nil
    watermarkLocation:LLWatermarkLocationCenter
...
- (void)exportVideoForAsset:(PHAsset *)asset type:(LLExportVideoType)type complete:(void (^)(NSString *, NSError *))complete
{
    [self exportVideoForAsset:asset type:type presetName:AVAssetExportPresetMediumQuality complete:complete];
}

- (void)exportVideoForAsset:(PHAsset *)asset type:(LLExportVideoType)type presetName:(NSString *)presetName complete:(void (^)(NSString *, NSError *))complete
{
    [self export:asset type:type presetName:presetName renderSize:CGSizeZero watermarkImage:nil watermarkLocation:LLWatermarkLocationCenter imageSize:CGSizeZero effectImage:nil birthRate:0 velocity:0 complete:complete];
}

- (void)exportVideoForAsset:(PHAsset *)asset type:(LLExportVideoType)type renderSize:(CGSize)renderSize complete:(void (^)(NSString *, NSError *))complete
{
    [self exportVideoForAsset:asset type:type renderSize:renderSize watermarkImage:nil watermarkLocation:LLWatermarkLocationCenter imageSize:CGSizeZero complete:complete];
}

#pragma mark - 视频加水印及粒子效果
- (void)exportVideoForAsset:(PHAsset *)asset type:(LLExportVideoType)type renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage watermarkLocation:(LLWatermarkLocation)location imageSize:(CGSize)imageSize complete:(void (^)(NSString *, NSError *))complete
{
...
- (void)exportVideoForAsset:(PHAsset *)asset type:(LLExportVideoType)type presetName:(NSString *)presetName watermarkImage:(UIImage *)watermarkImage watermarkLocation:(LLWatermarkLocation)location imageSize:(CGSize)imageSize complete:(void (^)(NSString *, NSError *))complete
{
...
- (void)exportVideoForAsset:(PHAsset *)asset type:(LLExportVideoType)type presetName:(NSString *)presetName effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity complete:(void (^)(NSString *, NSError *))complete
{
    [self export:asset type:type presetName:presetName renderSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) watermarkImage:nil watermarkLocation:LLWatermarkLocationCenter imageSize:CGSizeZero effectImage:effectImage birthRate:birthRate velocity:velocity complete:complete];
}

//privite
- (void) export:(PHAsset *)asset type:(LLExportVideoType)type presetName:(NSString *)presetName renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage watermarkLocation:(LLWatermarkLocation)location imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity complete:(void (^)(NSString *, NSError *))complete
{
...
- (void) export:(AVAsset *)asset range:(CMTimeRange)range type:(LLExportVideoType)type presetName:(NSString *)presetName renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage watermarkLocation:(LLWatermarkLocation)location imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity complete:(void (^)(NSString * exportFilePath, NSError * error))complete
{
    ...
    exportSession.outputFileType = (type == LLExportVideoTypeMov ? AVFileTypeQuickTimeMovie : AVFileTypeMPEG4);
...
- (AVMutableVideoComposition *)getVideoComposition:(AVAsset *)asset renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage watermarkLocation:(LLWatermarkLocation)location imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity
{
...
- (void)addWatermark:(AVMutableVideoComposition *)videoCom renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage watermarkLocation:(LLWatermarkLocation)location imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity
{
...
- (NSString *)getVideoExportFilePath:(LLExportVideoType)type
{
    NSString * format = (type == LLExportVideoTypeMov ? @"mov" : @"mp4");

    NSString * exportFilePath = [[NSString ll_libCachePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@_%@.%@", [self getUniqueStrByUUID],[NSDate getNowTimeTimesSeconds], format]];
