...
#import "LLPhotoBrowserEditBottomBar.h"
#import "LLPhotoPreviewCell.h"
#import "LLPhotoEditVideoViewController.h"
#import "LLPhotoBrowserManager.h"
#import <Photos/Photos.h>

@interface LLPhotoEditPhotoViewController () <UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UINavigationControllerDelegate> {
...
@property (nonatomic, strong) LLPhotoBrowserEditBottomBar * bottomBar;

@end

@implementation LLPhotoEditPhotoViewController {
...
- (void)setupUI
{
    ...
    [_collectionView registerClass:[LLPhotoPreviewCell class] forCellWithReuseIdentifier:@"LLPhotoPreviewCell"];
    ...
    [topPreview mas_makeConstraints:^(MASConstraintMaker * make) {
        ...
        make.height.mas_equalTo(LLTopHeight);
    }];
    ...
    [_navRightButton setImage:[UIImage ll_loadBundleImage:@"icon_radio_normal" ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"] forState:UIControlStateNormal];
    [_navRightButton setImage:[UIImage ll_loadBundleImage:@"icon_radio_selected" ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"] forState:UIControlStateSelected];
...
- (void)dataContainerInitialization
{
    ...
    for (LLPhotoBrowserModel * model in self.selectedModels) {
...
- (void)editWithOriginalImageWithSelect:(BOOL)isSelect
{
    ...
    LLPhotoBrowserModel * model = [self photoBrowserModel];
    if ([_imageArray containsObject:model.image]) {
        [_imageArray removeObject:model.image];
    }
    if (isSelect) {
        WeakSelf(weakSelf);
        [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestOriginalImageForAsset:model.asset
...
- (void)edit
{
    ...
    LLPhotoPreviewCell * cell   = [_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
    LLPhotoBrowserModel * model = self.models[index];

    WeakSelf(weakSelf);
    if (model.type == LLAssetMediaTypeImage) {
        LLImageCropViewController * imageCrop = [[LLImageCropViewController alloc] initWithImage:cell.previewView.imageGifView.image];
        ...
        LLPhotoEditVideoViewController * video = [[LLPhotoEditVideoViewController alloc] init];
        video.model                             = model;
        video.editVideoFinishBlock              = ^(LLPhotoBrowserModel * _Nonnull videoModel) {
...
- (void)rightBtnClick
{
    ...
    if (model.selected) {
        ...
        if (model.type == LLAssetMediaTypeVideo) {
            [_videoAssetArray addObject:model.asset];
        } else if (model.type == LLAssetMediaTypeImage) {
...
- (void)resetEditBtnStateWithModel:(LLPhotoBrowserModel *)model
{
    if ([self photoBrowserModel].type == LLAssetMediaTypeImage) {
        if ([self config].allowEditImage == NO) {
            self.bottomBar.editButton.hidden = YES;
        } else {
            self.bottomBar.editButton.hidden = NO;
        }
    } else if ([self photoBrowserModel].type == LLAssetMediaTypeVideo) {
        if ([self config].allowEditVideo == NO) {
            self.bottomBar.editButton.hidden = YES;
        } else {
            if (model.videoLength < [self config].maxVideoDuration) {
                self.bottomBar.editButton.hidden = YES;
            } else {
                self.bottomBar.editButton.hidden = NO;
            }
...
{
    if ([self photoBrowserModel].type == LLAssetMediaTypeImage && [self config].allowSelectOriginal == YES) {
...
{
    LLPhotoPreviewCell * cell   = [collectionView dequeueReusableCellWithReuseIdentifier:@"LLPhotoPreviewCell" forIndexPath:indexPath];
    LLPhotoBrowserModel * model = self.models[indexPath.row];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    _navRightButton.selected     = model.selected;
    [self resetDontBtnState];
    [self resetEditBtnStateWithModel:model];
...
#pragma mark ======= Getter
- (LLPhotoBrowserEditBottomBar *)bottomBar
{
    if (!_bottomBar) {
        _bottomBar = ({
            LLPhotoBrowserEditBottomBar * object = [[LLPhotoBrowserEditBottomBar alloc] init];
            WeakSelf(weakSelf);
            object.editBlock = ^{
                [weakSelf edit];
            };
...
- (LLPhotoBrowserModel *)photoBrowserModel
{
