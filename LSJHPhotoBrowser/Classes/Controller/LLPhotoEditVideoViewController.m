//
//  LLPhotoEditVideoViewController.m
//
//  Created by 李世航 on 2019/5/22.
//

#import "LLPhotoEditVideoViewController.h"
...
///////-----cell
@interface LLEditVideoCell : UICollectionViewCell

@property (nonatomic, strong) UIImageView * imageView;

@end

@implementation LLEditVideoCell
...
@protocol LLEditFrameViewDelegate <NSObject>
...
///////-----编辑框
@interface LLEditFrameView : UIView {
    UIImageView * _leftView;
    UIImageView * _rightView;
}

@property (nonatomic, assign) CGRect validRect;
@property (nonatomic, weak) id<LLEditFrameViewDelegate> delegate;

@end

@implementation LLEditFrameView
...
- (void)setupUI
{
    ...
    _leftView                        = [[UIImageView alloc] initWithImage:[UIImage ll_loadBundleImage:@"zl_ic_left" ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"]];
    ...
    _rightView                        = [[UIImageView alloc] initWithImage:[UIImage ll_loadBundleImage:@"zl_ic_right" ClassName:NSStringFromClass(self.class) bundleName:@"LSJHPhotoBrowser"]];
...
- (void)panAction:(UIGestureRecognizer *)pan
{
    ...
    const CGFloat W = self.cmam_width;
    CGFloat minX    = 0;
    CGFloat maxX    = W;
...
@interface LLPhotoEditVideoViewController () <UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, LLEditFrameViewDelegate> {
    UIView * _bottomView;
    UIButton * _cancelBtn;
    UIButton * _doneBtn;
...
@property (nonatomic, strong) LLEditFrameView * editView;
@property (nonatomic, strong) AVAssetImageGenerator * generator;

@end

@implementation LLPhotoEditVideoViewController
...
- (void)viewDidLayoutSubviews
{
    ...
    self.playerLayer.frame = CGRectMake(15, inset.top > 0 ? inset.top : 30, ScreenWidth - 30, ScreenHeight - 160 - inset.bottom);

    self.editView.frame       = CGRectMake((ScreenWidth - kItemWidth * 10) / 2, ScreenHeight - 100 - inset.bottom, kItemWidth * 10, kItemHeight);
    self.editView.validRect   = self.editView.bounds;
    self.collectionView.frame = CGRectMake(inset.left, ScreenHeight - 100 - inset.bottom, ScreenWidth - inset.left - inset.right, kItemHeight);

    CGFloat leftOffset  = ((ScreenWidth - kItemWidth * 10) / 2 - inset.left);
    CGFloat rightOffset = ((ScreenWidth - kItemWidth * 10) / 2 - inset.right);
    ...
    _bottomView.frame   = CGRectMake(0, ScreenHeight - bottomViewH - inset.bottom, ScreenWidth, kItemHeight);
    _cancelBtn.frame    = CGRectMake(10 + inset.left, 7, 40, bottomBtnH);
    _doneBtn.frame      = CGRectMake(ScreenWidth - 70 - inset.right, 7, 60, bottomBtnH);
}
...
- (void)setupUI
{
    ...
    [self.collectionView registerClass:LLEditVideoCell.class forCellWithReuseIdentifier:@"LLEditVideoCell"];

    [self.view addSubview:self.collectionView];

    [self creatBottomView];

    self.editView          = [[LLEditFrameView alloc] init];
...
- (void)analysisAssetImages
{
    ...
    WeakSelf(weakSelf);
    [[LLPhotoBrowserManager sharedPhotoBrowserManager] requestVideoForAsset:self.model.asset
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:self.model.asset
                                                    options:options
...
- (void)cancelBtn_click
{
    ...
    LLPhotoBrowser * nav = (LLPhotoBrowser *)self.navigationController;
...
- (void)btnDone_click
{
    ...
    [UIView ll_toastWithMessage:@"保存视频中" imageString:@"spin" autoRotation:YES imageType:LLToastImageTypeSVG sourceInKitBundle:YES autoDismiss:NO bgViewUserInteractionUse:NO];
    LLPhotoBrowser * nav = (LLPhotoBrowser *)self.navigationController;

    WeakSelf(weakSelf);
    __weak typeof(nav) weakNav = nav;
    [[LLPhotoBrowserManager sharedPhotoBrowserManager] exportEditVideoForAsset:_avAsset
                                                                          range:[self getTimeRange]
                                                                           type:[self config].exportVideoType
                                                                       complete:^(BOOL isSuc, PHAsset * asset) {
                                                                           [UIView ll_dismissToast];
                                                                           StrongSelf(strongSelf);
                                                                           if (isSuc) {
                                                                               __strong typeof(weakNav) strongNav = weakNav;
                                                                               LLPhotoBrowserModel * model       = [LLPhotoBrowserModel modelWithAsset:asset type:LLAssetMediaTypeVideo duration:nil];
                                                                               // 把编辑完成的视频回传至上一个页面
                                                                               ...
                                                                               [strongSelf startTimer];
                                                                               [UIView ll_showCenterToastWithMessage:@"视频保存失败"];
                                                                           }
                                                                       }];
}
...
#pragma mark ========= LLEditFrameViewDelegate
- (void)editViewValidRectChanged
{
    [self stopTimer];
    [self.playerLayer.player seekToTime:[self getStartTime] toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
...
{
    LLEditVideoCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"LLEditVideoCell" forIndexPath:indexPath];
...
static const char _LLOperationCellKey;
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        if (!error && cgImg) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (row == nowIndexPath.row) {
                    [(LLEditVideoCell *)cell imageView].image = image;
                } else {
                    UIImage * cacheImage = self->_imageCache[@(nowIndexPath.row).stringValue];
                    if (cacheImage) {
                        [(LLEditVideoCell *)cell imageView].image = cacheImage;
                    }
                }
            });
    ...
    objc_setAssociatedObject(cell, &_LLOperationCellKey, op, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
    NSBlockOperation * op = objc_getAssociatedObject(cell, &_LLOperationCellKey);
    if (op) {
        [op cancel];
        objc_removeAssociatedObjects(cell);
